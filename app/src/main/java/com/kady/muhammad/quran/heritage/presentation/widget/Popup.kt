...
import com.kady.muhammad.quran.heritage.databinding.PopupBinding
...
class Popup @JvmOverloads constructor(
    ...
    private val binding: PopupBinding
    ...
    init {
        ...
        binding = DataBindingUtil.inflate(inflater, R.layout.popup, this, true)
        ...
        setOnTouchListener { _, event ->
            if (event.action == MotionEvent.ACTION_DOWN) {
                ...
                cardView.getGlobalVisibleRect(recyclerViewRect)
    ...
    fun show(anchorView: View) {
        ...
        post {
            cardView.x = anchorView.x + 8F.px
            cardView.y = anchorView.y + statusBarHeight + 8F.px
        }
        cardView.animateProperty(ViewProperty.HEIGHT, duration = 350)
        postDelayed({
            //
            requestFocus()
        }, 100)
    ...
    private fun hide() {
        isPopupShown = false
        cardView.animateProperty(ViewProperty.HEIGHT, true, duration = 350) {
            content.removeView(this)
        }
    }

    private inner class ItemsAdapter(val items: MutableList<String> = mutableListOf()) :
        ...
        fun updateAdapter(items: List<String>) {
            this.items.clear()
